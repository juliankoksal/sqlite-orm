...
std::int64_t Handle::insert(const Object& dbObject) const
{
    ...
    bindStatementColumns(statement, dbObject, dbObject.columns(), index,
                         "insert");
...
void Handle::update(const Object& dbObject) const
{
    ...
    int index = 1; // SQL statement parameter index.
    bindStatementColumns(statement, dbObject, columnsToBind, index, "update");
    bindStatementColumns(statement, dbObject, keysToBind, index, "update");
                         ...
                         const std::set<std::string>& columns) const
{
    ...
    int index = 1; // SQL statement parameter index.
    bindStatementColumns(statement, dbObject, columnsToBind, index, "updateWhere");
...
void Handle::destroy(const Object& dbObject) const
{
    ...
    int index = 1; // SQL statement parameter index.
    bindStatementColumns(statement, dbObject, keysToBind, index, "destroy");
                                          ...
                                          const std::set<std::string>& columns) const
{
    ...
    {
        std::map<std::string, std::any> row = dbObject.toMap();
...
sqlite3_stmt* Handle::prepareStatement(const std::string& query,
                                       const std::string& queryType) const
{
    ...
    if (result != SQLITE_OK)
    {
        ...
        throw std::runtime_error("Error preparing " + queryType
                                 + " statement. SQLite3 error "
...
void Handle::bindStatementColumns(sqlite3_stmt* statement,
                                  ...
                                  const std::string& queryType) const
{
    std::map<std::string, std::any> mMap = dbObject.toMap();
    
    for (auto it = columns.cbegin(); it != columns.cend(); ++it)
    {
        int result = -1;
        
        if (queryType == "insert" && dbObject.isAutoGeneratedKey()
        ...
        if (result != SQLITE_OK)
        {
            ...
            if (result == -1)
            {
                throw std::runtime_error("Error in call to DBHelper::"
                                         + queryType + "(). The value of '"
                                         + *it
                                         + "' is not a supported data type.");
            }
            throw std::runtime_error("Error binding " + queryType
                                     + " statement. SQLite3 error "
                                     + std::to_string(result) + ": "
                                     + std::string(sqlite3_errmsg(db)));
        }
                                     ...
                                     const std::string &queryType) const
{
    for (auto it = conditions.cbegin(); it != conditions.cend(); ++it)
    {
        ...
        const std::any columnValue = it->value;
        const std::type_info* attrType = &(columnValue.type());
        if (*attrType == typeid(std::int64_t))
        {
            result = sqlite3_bind_int64(statement, index,
        ...
        {
            const auto& in = std::any_cast<std::vector<std::int64_t>>(columnValue);
        ...
        {
            const auto& in = std::any_cast<std::vector<std::string>>(columnValue);
            for (auto itIn = in.cbegin(); itIn != in.cend(); ++itIn)
            {
...
std::string Handle::generateWhereClauseFromConditions(const std::vector<Condition>& conditions)
{
    ...
    for (auto it = conditions.cbegin(); it != conditions.cend(); it++)
    {
        ...
        if (op == "IN")
        {
            ...
            if (*attrType == typeid(std::vector<std::int64_t>))
            {
                std::size_t size = std::any_cast<std::vector<std::int64_t>>(it->value).size();
            ...
            {
                std::size_t size = std::any_cast<std::vector<std::string>>(it->value).size();
                for (std::size_t i = 0; i < size; ++i)
                {
                    result += "?,";
                }
...
const std::string Handle::opStr(Op op)
{
    using enum Op;
    switch (op)
    {
