...
DB::Object* MenuItem::fromMap(const std::map<std::string, std::any>& dbObjectMap) const
{
    static const std::vector<std::string>& cols = columns();
    
    std::string name = std::any_cast<std::string>(dbObjectMap.at(cols[0]));
    double price = std::any_cast<double>(dbObjectMap.at(cols[1]));
